...
class FormPage {
    ...
    openContactForm() {
        cy.visit(`${Cypress.env('siteURL')}/#contact_us`);
    }
    ...
    get submitBtn () {
        return cy.get('#sendForm')
    }

    get submitModal () {
        return cy.get('body > div.container > main > section.discover > div > div.modal-overlay > div')
    }

    get submitText () {
        return cy.get('body > div.container > main > section.discover > div > div.modal-overlay > div > h3')
    }

    get submitTextConfirmation () {
        return cy.get('body > div.container > main > section.discover > div > div.modal-overlay > div > p')
    }

    get submitCloseBtn () {
        return cy.get('body > div.container > main > section.discover > div > div.modal-overlay > div > button')
    }
    ...
    checkSubmitModal() {
        ...
        this.submitText
            .contains('Thank you for the demo request')
            .should('be.visible')

        this.submitTextConfirmation
            .contains('For more info please check your e-mail inbox.')
            .should('be.visible')
    }
